...
import GraficoUniformeArea from '../../../Graficas/GraficoUniformeArea';
import GraficoUniformeMosaico from '../../../Graficas/GraficoUniformeMosaico';
...
export default function ResultadosUniforme({
    ...
    const calcular = () => {
        ...
        const res = calcularDistribucionUniforme(bounds.min, bounds.max, inputMin, inputMax);
        if (res.error) {
            setError(res.error);
        } else {
            setResultado({
                ...res.resultado,
                dataReales: bounds.dataReales
            });
            setError('');
        }
    };
    ...
    return (
        <div style={{ marginTop: '0px', fontFamily: FONT }}>
            <div style={{ ...cardStyle, marginBottom: '20px' }}>
                ...
                <h4 style={{ color: 'var(--primary-color)', margin: '0 0 15px 0', fontSize: FS.sm }}>Parámetros de Distribución Uniforme:</h4>
                ...
                {numCols.length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', background: 'var(--bg-input)', padding: '15px', borderRadius: RADIUS, border: '1px solid var(--border-color)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                            <div style={{ flex: 1, minWidth: '150px' }}>
                                <label style={{ fontSize: FS.sm, display: 'block', marginBottom: '4px', fontWeight: 'bold', color: '#1e293b' }}>Mínimo (a):</label>
                            ...
                            <div style={{ flex: 1, minWidth: '150px' }}>
                                <label style={{ fontSize: FS.sm, display: 'block', marginBottom: '4px', fontWeight: 'bold', color: '#1e293b' }}>Máximo (b):</label>
                        ...
                        <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
                            <button onClick={calcular} disabled={!varUniforme || !inputMin || !inputMax} style={{ padding: '8px 30px', borderRadius: RADIUS, fontSize: FS.sm, fontWeight: 700, height: '38px', background: 'var(--primary-color)', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', opacity: (!varUniforme || !inputMin || !inputMax) ? 0.5 : 1 }}>
            ...
            {resultado && (
                <>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        <MarcoWidgetMAT251 id="w-uni-area" titulo="Distribución Uniforme Teórica y Área de Interés" anchoCompleto={true} alto="380px">
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        <MarcoWidgetMAT251 id="w-uni-mosaico" titulo="Mosaico Frecuencial (Datos Reales) vs. Modelo Teórico" anchoCompleto={true} alto="380px">
                            <div style={{ width: '100%', height: '100%', minWidth: 0, padding: '10px', overflowX: 'auto', overflowY: 'hidden' }}>
